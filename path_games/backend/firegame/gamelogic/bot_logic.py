...
from . import get_adj_indices,fire_step,state_logic
import numpy as np
import time
...
def move_bot_5(grid,botindex,botroute,safeguard):
    if grid[botindex]%10==2 or grid[botindex]%10==5:
        return grid,botindex,safeguard,botroute
    if botroute==[]:
        return grid,botindex,safeguard,botroute
    #safeguard==True means bot 4 is in bot 3 mode  
    if safeguard:
        grid,botindex,botroute = bot_5_does_bot_3(grid,botindex,botroute)
        return grid,botindex,safeguard,botroute
    else:
        #At every step, checks to see if bot 3 mode needed
        safeguard = bot_4_check_route(grid,botindex,botroute)
        if not safeguard:
            index = botroute.index(botindex)
            return grid,botroute[index+1],safeguard,botroute
        else:
            grid,botindex,plan = bot_5_does_bot_3(grid,botindex,botroute)
            return grid,botindex,safeguard,botroute

"""def move_bot_5(grid,botindex,botpath,t):
    if grid[botindex]%10==2 or grid[botindex]%10==5:
        return grid,botindex,botpath
    if t%4==0:
        weights = state_logic.predicting(grid,t)
        path = bot_4_UFCS(grid,botindex,weights)
        if len(path)<2:
            return grid,botindex,path
        return grid,path[1],path
    else:
        if len(botpath)==0:
            return grid,botindex,botpath
        botpath.pop(0)
        return grid,botpath[0],botpath
"""
...
def bot_5_UFCS(grid,start,weights):
        #turn 3d UFCS into 2d by making weights list of arrays into a hashmap/graph that UFCS algorithm can traverse
        #this entire function is also explained in detail in my write-up, so I will add no more comments
        newweights = {(0,start,0):None}
        seen = []
...
def bot_5_check_route(grid,start,route):
        #For every index along the route, check adjacent tiles to see if they're on fire. If so, return True, else return False
        try:
            index = route.index(start)
        except:
...
def bot_5_does_bot_3(grid,index,plan):
    #Literally just the bot 3 code with very few adjustments, I could've done it in one function but whatever.
    if bot_4_check_route(grid,index,plan):
        altgrid = grid.copy()
        for i in range(len(altgrid)):
            for j in range(len(altgrid)):
                if grid[(i,j)]==0:
        ...
        bfslist = bfs(altgrid,index)
        if not bfslist == []:
            return grid,bfslist[1],bfslist
        else:
            bfslist = bfs(grid,index)
            if not bfslist==[]:
                return grid,bfslist[1],bfslist
            else:
                return grid,index,bfslist
    else:
        index = plan.index
        return grid,plan[index+1],plan
